# 
# #INFO: Calculation of the time values for the main plot
# 
# -----------------------------------------------------------------------------
# 
# Author: DTMc
...
ber_data = np.zeros((len(snr), len(time)))
mi_data  = np.zeros((len(snr), len(time)))
cluster_data = np.zeros((3*len(time), 200))
header   = np.array(("SNR"))
...
for time_val in time:
    ...
    for SNR in snr:
        ...
        _init = {   
                ...
                "Mass"              : 1,
                "RNG"               : np.array([42, 89]),
                "Symbol length"     : 30 * time_val,
                "SNR"               : SNR,
                "Transmission Type" : "open",
                "Sampling Value"    : 30 * time_val,
                "Decoding Method"   : "OOK",
            } 
        ...
        if SNR == 20:
             # Save the d-axis transmission
             cluster_data[time_val - 1][:]  = Rx[0][:]

             # Save the q-axis transmission
             cluster_data[time_val - 1 + len(time)][:]  = Rx[1][:]

             cluster_data[time_val - 1 + 2*len(time)][:] = \
                 np.insert(np.delete(Decoded_Tx[:],-1,-1),0,0)

        ber_data[snr_index][time_index] = \
            mc.BER(Decoded_Rx, Decoded_Tx) / _init["bit length"]
        
        # We finally calculate the mutual information
        mi_data[snr_index][time_index] =  mc.mutual_information(
            Decoded_Tx,
            Decoded_Rx,
            _init["Level count"])
    ...
    header = np.append(header, str("TS=" + str(time_val  * 30)))
...
mc.util_save_to_csv(str(SAVE_CSV_PATH + "mi-time.csv"),      mi_output)
mc.util_save_to_csv(str(SAVE_CSV_PATH + "ber-time.csv"),     ber_output)
mc.util_save_to_csv(str(SAVE_CSV_PATH + "cluster-time.csv"), np.transpose(cluster_data))

# ------------------------------------------------------------------------------
# molcom_time.py ends here.
# 
# 
