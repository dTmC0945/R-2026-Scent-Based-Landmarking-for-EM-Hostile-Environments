# 
# #INFO: Calculation of dist values for the main plot
# 
# -----------------------------------------------------------------------------
# 
# Author: DTMc
...
ber_data = np.zeros((len(snr), len(dist)))

mi_data = np.zeros((len(snr), len(dist)))

cluster_data = np.zeros((3*len(dist), 1000))
...
for dist_val in dist:
    ...
    for SNR in snr:
        ...
        if SNR == 30:
             ...
             cluster_data[dist_val - 1][:]  = Rx[0][:]

             # Save the q-axis transmission
             cluster_data[dist_val - 1 + len(dist)][:]  = Rx[1][:]

             cluster_data[dist_val - 1 + 2*len(dist)][:] = \
                 np.insert(np.delete(Decoded_Tx[:],-1,-1),0,0)


        ber_data[snr_index][dist_index] = \
            mc.BER(Decoded_Rx, Decoded_Tx) / _init["bit length"]
        
        # We finally calculate the mutual information
        mi_data[snr_index][dist_index] = \
    ...
    header = np.append(header, str("Da / Db=" + str(dist_val)))
...
mc.util_save_to_csv(str(SAVE_CSV_PATH + "mi-dist.csv"), mi_output)
mc.util_save_to_csv(str(SAVE_CSV_PATH + "ber-dist.csv"), ber_output)
mc.util_save_to_csv(str(SAVE_CSV_PATH + "cluster-dist.csv"), np.transpose(cluster_data))


# ------------------------------------------------------------------------------
#  molcom_dist.py ends here.
# 
# 
